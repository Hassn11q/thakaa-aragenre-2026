...
| `trap_arm_b.json` | `src/ablate_trap.py` | arm B: broad definitions plus the explicit type-versus-topic instruction. Arm C is `verify_gpt.json`. `src/trap_table.py` rebuilds the 394/233/119 table from these files without any API access. Arm B was run after the evaluation phase closed, so that comparison is post hoc. |
