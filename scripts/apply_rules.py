"""Apply the submitted surface rules after the family judge.

A few orthographic and formatting patterns identify a genre with high precision. When one
matches, the rule replaces both labels; otherwise it keeps the judge prediction. Examples include
Quranic orthography, hadith isnad openings, and short interactive posts.

Usage:
  python scripts/apply_rules.py --pred work/predictions.json --out work/predictions_with_rules.json
  python scripts/apply_rules.py --pred <p> --measure     # report rule coverage and changed predictions
"""
...
# Interactive-recall rule: the gate under-predicts Interactive; short
...
OPIN = re.compile(rf"(?<![{AB}])(?:رائع|روعة|ممل|سيء|سيئ|جميل|أنصح|ما شاء الله|تحفة|مبدع|فنان|رهيب|زفت|"
...
APP_CUE = re.compile(rf"(?:التطبيق|تطبيق|البرنامج|التطبيقات)")
BOOK_CUE = re.compile(rf"(?:الكتاب|الرواية|رواية|الكاتب|الكاتبة|كتاب)")
...
def interactive_rule(t):
    ...
    wc = len(t.split())
    fire = has_hash or (len(t) <= 130 and has_emo and has_opin) or (len(t) <= 90 and has_opin and wc <= 14)
    if not fire:
        return None
    if has_hash or "@" in t:
        spec = "twitter_posts"
...
def main():
    ...
    for p in preds:
        ...
        if r:
            ...
            p = {"id": p["id"], "broad_genre": s2b[s], "specific_genre": s}
        out.append(p)
