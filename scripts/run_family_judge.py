"""Run the family-restricted specific-genre judge.
...
TOPK_GLOB = "retrieval_topk_*of*.json"
TEST = ROOT / "test" / "test.json"
DEFS = ROOT / "test" / "test_genre_definitions.json"
BROAD_SRC = Path(os.environ.get("BROAD_SRC", ROOT / "cached_outputs" / "family_gate_predictions.json"))
SPEC_FALLBACK = Path(os.environ.get("SPEC_FALLBACK", ROOT / "cached_outputs" / "family_gate_predictions.json"))
WORK_DIR = Path(os.environ.get("WORK_DIR", ROOT / "work"))
OUT = Path(os.environ.get("OUT", WORK_DIR / "family_judge_predictions.json"))
SCORES = Path(os.environ.get("SCORES", WORK_DIR / "family_judge_scores.json"))
ALPHA = float(os.environ.get("ALPHA", "0.75"))
MAX_ENUM = int(os.environ.get("MAX_ENUM", "12"))            # cap candidates shown to the judge
TEXT_CAP = 6000
...
def setwise_scores(text, defs, k, cue=""):
    ...
    for attempt in range(3):                             # retry transient errors under high concurrency
        try:
            ...
            if attempt == 2:
                print(f"[judge] request failed after 3 attempts: {exc}", file=sys.stderr)
    if txt is None:
        return None
...
def main():
    ...
    enriched_file = WORK_DIR / "enriched_definitions.json"
    if os.environ.get("ENRICHED_DEFS") == "1" and enriched_file.exists():
        defmap = {d["specific_genre"]: d["enriched_definition"] for d in json.loads(enriched_file.read_text())}
    elif enrich:
        sys.path.insert(0, str(Path(__file__).resolve().parent))
        from discriminators import DISC
        defmap = {d["specific_genre"]: d["specific_genre_definition"] +
                  (f" | كيف تميّزه: {DISC[d['specific_genre']]}" if d["specific_genre"] in DISC else "")
                  for d in defrows}
    else:
        defmap = {d["specific_genre"]: d["specific_genre_definition"] for d in defrows}
    s2b = {d["specific_genre"]: d["broad_genre"] for d in defrows}
    fam = defaultdict(list)
    for d in defrows:
        fam[d["broad_genre"]].append(d["specific_genre"])

    broad_by_id = {x["id"]: x["broad_genre"] for x in json.loads(BROAD_SRC.read_text())}
    spec_v2_by_id = {x["id"]: x["specific_genre"] for x in json.loads(SPEC_FALLBACK.read_text())}
    text_by_id = {r["id"]: (r.get("text") or "")[:TEXT_CAP] for r in json.loads(TEST.read_text())}

    top20 = {}
    for f in sorted(WORK_DIR.glob(TOPK_GLOB)):
        for it in json.loads(f.read_text())["topk"]:
            top20[it["id"]] = it["top_genres"]
    ids = [r["id"] for r in json.loads(TEST.read_text())]

    # Tight retrieval-pruned candidates outperformed enumerating whole large families.
    # DIALECT/TEXT-TYPE families (Creative songs, Religious) are NOT separable by retrieval over English
    # definitions. The judge reads the Arabic and decides, so show all of them (at most seven). Small
    # Interactive/Legal: show all. TOPIC families (Informative, Learning): keep only the top-KTOP
    # retrieved in-family (floor handled by availability).
    SHOW_ALL = {"Creative", "Religious", "Interactive", "Legal"}
    KTOP = int(os.environ.get("KTOP", "6"))
    def candidates(rid):
        b = broad_by_id[rid]
        sibs = fam[b]
        if b in SHOW_ALL:
        ...
        if not infam:
            return [spec_v2_by_id[rid]]
        return infam[:KTOP]

    PASSES = int(os.environ.get("PASSES", "1"))   # permutation self-consistency: rotate candidate order
    def work(rid):
        ...
        acc = np.zeros(k, dtype=float); n = 0
    ...
    for g, sc in per.values():
        for gi, v in zip(g, sc):
            tot[gi] = tot.get(gi, 0.0) + v; cnt[gi] = cnt.get(gi, 0) + 1
    ...
    SCORES.write_text(json.dumps(
        [{"id": rid, "genres": per[rid][0], "scores": [round(float(x), 1) for x in per[rid][1]]}
         for rid in ids], ensure_ascii=False))
    print(f"[hier] wrote {len(preds)} -> {OUT}; MAX_ENUM={MAX_ENUM} ALPHA={ALPHA}")
    print("[hier] specific top12:", Counter(p["specific_genre"] for p in preds).most_common(12))
